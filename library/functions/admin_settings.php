		...
		if ( 'save' == $_REQUEST['action'] ) {
				...
				if ( $value['type'] == 'file' ) {
					if ( $_FILES ) {
						...
						if ( $_FILES[ $image_var ]['name'] && $_FILES[ $image_var ]['size'] > 0 ) {
							...
							$files_allow = apply_filters( 'tmpl_allow_extra_file_upload',array( '.jpg', '.JPG', 'jpeg', 'JPEG', '.png', '.PNG', '.gif', '.GIF', '.jpe', '.JPE' ) );
							$file_ext = strtolower( substr( $target_path, -4, 4 ) );
							$fileinfo = getimagesize( $_FILES[ $image_var ]['tmp_name'] );
							if ( ! empty( $fileinfo ) ) {
			...
			foreach ( $options as $value ) {
				if ( $value['type'] != 'multicheck' ) {
					...
					foreach ( $value['options'] as $mc_key => $mc_value ) {
						$up_opt = $value['id'] . '_' . $mc_key;
						if ( isset( $_REQUEST[ $up_opt ] ) ) { update_option( $up_opt, $_REQUEST[ $up_opt ] );
						} else { delete_option( $up_opt ); }
					}
				}
			}
	...
	<?php foreach ( $options as $value ) {
		...
		switch ( $value['type'] ) {
			...
			case 'file':
				...
				global $thumb_url;
			?>
	  <input class="text_input" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="text" value="<?php if ( get_settings( $value['id'] ) != '' ) { echo get_settings( $value['id'] );
} else { echo $value['std']; } ?>" />
	...
	<?php }?>
		...
		break;
			...
			case 'text':
				option_wrapper_header( $value );
			?>
	  <input class="text_input" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_settings( $value['id'] ) != '' ) { echo stripslashes( get_settings( $value['id'] ) );
} else { echo $value['std']; } ?>" />
		<?php
		option_wrapper_footer( $value );
		break;
...
} elseif ( $option == $value['std'] ) { echo ' selected="selected"'; } ?>><?php echo $option; ?></option>
	<?php } ?>
	  </select>
		<?php
		option_wrapper_footer( $value );
		break;
			...
			case 'textarea':
				...
				option_wrapper_header( $value );
			?>
	  <textarea name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" cols="<?php echo $ta_options['cols']; ?>" rows="8"><?php  if ( get_settings( $value['id'] ) != '' ) { echo stripslashes( get_settings( $value['id'] ) );
} else { echo $value['std']; } ?>
	</textarea>
		<?php
		option_wrapper_footer( $value );
		break;
			...
			case 'textarea2':
				...
				option_wrapper_header( $value );
			?>
	  <textarea name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" cols="<?php echo $ta_options['cols']; ?>" rows="2"><?php  if ( get_settings( $value['id'] ) != '' ) { echo stripslashes( get_settings( $value['id'] ) );
} else { echo $value['std']; } ?>
	</textarea>
		<?php
		option_wrapper_footer( $value );
