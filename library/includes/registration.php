...
if ( $_REQUEST['back'] == 1 ) {
	$register_user_information = $_SESSION['register_user_information'];
	$user_email = $register_user_information['user_email'];
	$user_pwd = $register_user_information['user_pwd'];
	$user_fname = $register_user_information['user_fname'];
	$user_web = $register_user_information['user_web'];
	$user_phone = $register_user_information['user_phone'];
	$user_twitter = $register_user_information['user_twitter'];
	$user_facebook = $register_user_information['user_facebook'];
} else {
	$_SESSION['register_user_information'] = array();
}
if ( $_REQUEST['action'] == 'register' && $_POST['user_email'] && $_POST['user_pwd'] && $_POST['user_fname'] && ( isset( $_POST['register_nonce_field'] ) || wp_verify_nonce( $_POST['register_nonce_field'], 'register_action' ) ) ) {

	if ( function_exists( 'pt_check_captch_cond' ) ) {
		if ( ! pt_check_captch_cond() && $_REQUEST['emsg'] == '' ) {
			$_SESSION['register_user_information'] = $_POST;
			wp_redirect( get_option( 'siteurl' ) . '/?ptype=login&page1=sign_up&back=1&emsg=captch' );
			exit;
		}
	}
	...
	if ( in_array( 'User Registration Page',$pcd ) || in_array( 'All of them',$pcd ) ) {
		...
		if ( ! $responde_encode->success ) {
			 wp_redirect( get_option( 'siteurl' ) . '?ptype=login&page1=sign_up&emsg=captch' );
			exit;
		}
	}
}
...
if ( is_user_logged_in() && $_REQUEST['action'] != 'logout' ) {
	wp_redirect( get_option( 'siteurl' ) . '/?ptype=account' );
	exit;
}
...
if ( force_ssl_admin() && ! is_ssl() ) {
	if ( 0 === strpos( $_SERVER['REQUEST_URI'], 'http' ) ) {
		wp_redirect( preg_replace( '|^//|', 'https://', $_SERVER['REQUEST_URI'] ) );
		exit();
	} else {
...
function retrieve_password() {
	...
	$message .= '<p>You can login to : <a href="' . get_option( 'siteurl' ) . '/?ptype=login' . '">Login</a> or the URL is :  ' . get_option( 'siteurl' ) . '/?ptype=login</p>';
...
function templ_register_new_user( $user_login, $user_email, $user_login1reg, $user_fname ) {
	global $wpdb;
	$errors = new WP_Error();

	$user_login1reg = $user_login1reg;
	$user_login = $user_email;
	$user_email = apply_filters( 'user_registration_email', $user_email );
	$user_user_fname = $wpdb->get_row( "SELECT * FROM $wpdb->users WHERE display_name = '$user_fname'" );
	$user_fname_count = count( $user_user_fname->ID );
	// Check the username
	if ( $user_login1reg == '' ) {
		$errors->add( 'empty_username', __( 'ERROR: Please enter a username.' ) );
	} elseif ( ! validate_username( $user_login1reg ) ) {
		$errors->add( 'invalid_username', __( '<strong>ERROR</strong>: This username is invalid.  Please enter a valid username.' ) );
		$user_login1reg = '';
	} elseif ( username_exists( $user_login1reg ) ) {
	...
	if ( $user_fname_count > 0 ) {
		$errors->add( 'fullname_exists', __( '<strong>ERROR</strong>: Full name already exists, Please try another.' ) );
	}
	...
	do_action( 'register_post', $user_login1reg, $user_email, $errors );
	...
	$user_id = wp_create_user( $user_login1reg, $user_pass, $user_email );
	...
	if ( $user_web && ( ! strstr( $user_web,'//' ) || ! strstr( $user_web,'http' )) ) {
		$user_web = '//' . $user_web;
	}
		$user_twitter = $_POST['user_twitter'];
	if ( $user_twitter && ( ! strstr( $user_twitter,'//' ) || ! strstr( $user_twitter,'http' )) ) {
		$user_twitter = '//' . $user_twitter;
	}
		$user_facebook = $_POST['user_facebook'];
	if ( $user_facebook && ( ! strstr( $user_facebook,'//' ) || ! strstr( $user_facebook,'http' )) ) {
	...
	$user_address_info = array(
						...
						'last_name'		=> sanitize_text_field( $_POST['user_lname'] ),
	...
	if ( $user_id ) {
		...
		<p>Thank you for becoming our valued member</p>
		<p>[#$store_name#]</p>');
		} else {
		...
		$replace_array = array( $_POST['user_fname'],$user_login1reg,$user_pass,$store_name,$store_login,$store_login_link );
...
if ( defined( 'RELOCATE' ) ) { // Move flag is set
	...
	$schema = ( isset( $_SERVER['HTTPS'] ) && strtolower( $_SERVER['HTTPS'] ) == 'on' ) ? 'https://' : '//';
...
switch ( $action ) {
	...
	case 'logout' :
		...
		//$redirect_to = get_option( 'siteurl' ).'/?ptype=login&loggedout=true';
	...
	case 'retrievepassword' :
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				wp_redirect( get_option( 'siteurl' ) . '/?ptype=login&page1=sign_in&checkemail=confirm' );
				exit();
			} else {
				wp_redirect( get_option( 'siteurl' ) . '/?ptype=login&page1=sign_in&emsg=fw' );
				exit();
			}
		}
...
break;
	...
	case 'rp' :
		...
		if ( ! is_wp_error( $errors ) ) {
			wp_redirect( get_option( 'siteurl' ) . '/?ptype=login&action=login&checkemail=newpass' );
			exit();
		}

		wp_redirect( get_option( 'siteurl' ) . '/?ptype=login&action=lostpassword&page1=sign_in&error=invalidkey' );
	exit();
	...
	case 'register' :
		...
		$user_email = '';
		if ( $http_post ) {
			$user_login1reg = sanitize_text_field( $_POST['user_login1reg'] );
			$user_login = $_POST['user_email'];
			$user_email = $_POST['user_email'];
			$user_fname = sanitize_text_field( $_POST['user_fname'] );
			$user_lname = sanitize_text_field( $_POST['user_lname'] );
			$user_add1 = sanitize_text_field( $_POST['user_add1'] );
			$user_add2 = sanitize_text_field( $_POST['user_add2'] );
			...
			$user_postalcode = sanitize_text_field( $_POST['user_postalcode'] );
			$user_twitter = sanitize_text_field( $_POST['user_twitter'] );
			$user_facebook = sanitize_text_field( $_POST['user_facebook'] );
			...
			$errors = templ_register_new_user( $user_login, $user_email, $user_login1reg,$user_fname );
			$_SESSION['error'] = $errors->errors;
			if ( is_wp_error( $errors ) ) {
				wp_redirect( get_option( 'siteurl' ) . '?ptype=login&page1=sign_up&emsg=1' );
			}
			if ( ! is_wp_error( $errors ) ) {
				unset( $_SESSION['error'] );
				$_POST['log'] = $user_login1reg;
				$_POST['pwd'] = $errors[1];
				$_POST['testcookie'] = 1;
...
break;
	...
	default:
		...
		if ( isset( $_POST['testcookie'] ) && empty( $_COOKIE[ TEST_COOKIE ] ) ) {
			$errors->add( 'test_cookie', __( "<strong>ERROR</strong>: Cookies are blocked or not supported by your browser. You must <a href='//www.google.com/cookies.html'>enable cookies</a> to use WordPress." ) );
		}
		...
		if ( ($_POST['log'] && $errors) || ($_POST['log'] == '' && $_REQUEST['testcookie']) ) {
			if ( $_REQUEST['pagetype'] ) {
				...
				wp_redirect( get_option( 'siteurl' ) . '?ptype=login&page1=sign_in&emsg=1' );
			}
			exit;
		}
break;
...
<?php } else { ?>
try{document.getElementById('user_login1reg').focus();}catch(e){}
<?php } ?>
</script>
<div id="content">
			<?php
			if ( $_REQUEST['page1'] == 'sign_up' ) {
				$page_title = REGISTRATION_NOW_TEXT;
			} else {
				$page_title = SIGN_IN_PAGE_TITLE;
			}
		?>

		  <h1 class="page_head"><?php _e( $page_title ); ?></h1>    
...
<?php
if ( isset( $_REQUEST['ptype'] ) && $_REQUEST['ptype'] == 'login' && $_REQUEST['page1'] == 'sign_in' ) {
?>
...
<?php
} elseif ( isset( $_REQUEST['ptype'] ) && $_REQUEST['ptype'] == 'login' && $_REQUEST['page1'] == 'sign_up' ) {
